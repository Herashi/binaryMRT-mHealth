...
)
{
  ...
  sample_size <- length(unique(as.matrix(dta[, id_varname])))
  ...
  A <- as.matrix(dta[, treatment_varname])
  A_delta <- as.matrix(dta[,paste0(treatment_varname,"_delta")])
  ...
  p_t <- as.matrix(dta[, rand_prob_varname])
  cA <- A - p_t # centered A
  Y <- as.matrix(dta[, outcome_varname])
  ...
  if (future_treatment == "ST"){
    weight_factor = ifelse(A_delta, 1/p_t, 0)
  ...
  ### 2. estimation ###
  person_first_index <- c(find_change_location(as.matrix(dta[, id_varname])), total_person_decisionpoint + 1)
  ...
  estimating_equation <- function(theta) {
    ...
    for (i in 1:sample_size) {
      ef_n[i, ] <- apply(ef[person_first_index[i] : (person_first_index[i+1] - 1),], 2, sum,na.rm = T)
    }
    ...
    for (g in group[["id"]]){
      ...
      if(g_size > 1){
        ef_g <-apply(ef_n[row_ind,],2,sum, na.rm = T)
      }else{
        ef_g = ef_n[row_ind,]
      }
      ef = ef+ef_g/g_size
  ...
  for (i in 1:sample_size) {
    Mn_n[i, , ] <- apply(Mn_summand[person_first_index[i] : (person_first_index[i+1] - 1),,], c(2,3), sum, na.rm = T)
  }
  ...
  for (g in group[["id"]]){
   ...
   g_size = group[["group size"]][g]
   if(g_size>1){
     bread[g,,] = apply(Mn_n[row_ind,,], c(2,3), sum, na.rm = T)/g_size
   }else{
     bread[g,,] = Mn_n[row_ind,,]
   }
  }
  
  midterm <- apply(bread, c(2,3), sum, na.rm = T)
